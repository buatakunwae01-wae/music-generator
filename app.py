...
if API_KEY:
    ...
    st.error("API Key belum dipasang. Silakan cek Setting > Secrets di Hugging Face.")
...
def analyze_audio(file_path):
    # Mengunggah file ke Gemini
    uploaded_file = genai.upload_file(path=file_path)
    
    # Tunggu sebentar agar file selesai diproses oleh Google
    time.sleep(3) 
    
    # Memilih otak AI (Gemini 1.5 Pro sangat pintar menganalisa suara)
    ...
    # Menghapus file di server Google setelah selesai agar bersih
    genai.delete_file(uploaded_file.name)
    
    return response.text

# --- TAMPILAN APLIKASI (MIRIP DENGAN DESAIN ANDA) ---
